...
import torch
import numpy as np
from config import ESM_MODEL_NAME

class ESMRandomForestPipeline:
    def __init__(self, model_name=ESM_MODEL_NAME, device=None):
    ...
    def _generate_inputset(self, inputset_dict):
        X = []
        for protein, seq in inputset_dict.items():
            embeddings = self.embed_sequence(seq)
        return np.array(X)

    def _generate_dataset(self, dataset_dict):
        """
